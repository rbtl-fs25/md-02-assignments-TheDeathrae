...
# https://stackoverflow.com/questions/5677885/ignore-outliers-in-ggplot2-boxplot?utm_source=chatgpt.com
```
...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here:\
    geom_col() uses the given y values, whereas geom_bar() as seen here doesn't have a y value input, it just counts the number of occurrences
...
6.  Describe how the `geom_histogram()` function is similar to the `geom_bar()` function.
    ...
    It also creates a bar plot, but in this case for example it displays more information, as it also shows number of occurrences, but spreads them over the corresponding lifeExp
...
7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe:
    ...
    The first two values seem to be the lower and upper boudary for the x-axis data display and the third value seems to be the step or width of the bars

```{r}

ggplot(gapminder_2007, aes(x = lifeExp, fill = continent)) +
  geom_histogram(col = "grey30", breaks = seq(40, 85, 2.50))
```
...
ggplot(gapminder_2007, aes(x = gdpPercap, y = lifeExp, size = pop, color = country)) +
